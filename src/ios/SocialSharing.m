...
@implementation SocialSharing {
  ...
  NSString *_popupCoordinates;
    NSString *_weibo_access_token;
    TencentOAuth *_tencent_auth;
}
...
- (void)pluginInitialize {
  if ([self isEmailAvailable]) {
    ...
    [myCenter addObserver:self selector:@selector(handleOpenURL:) name:CDVPluginHandleOpenURLNotification object:nil];
}
...
- (void)shareViaQq:(CDVInvokedUrlCommand *)command{
    ...
    NSString *subject   = [command.arguments objectAtIndex:1]!= (id)[NSNull null]?[command.arguments objectAtIndex:1]:@"";
    NSArray  *filenames = [command.arguments objectAtIndex:2];
    bool toZone=[command.arguments objectAtIndex:4]!= (id)[NSNull null]? ![[command.arguments objectAtIndex:4] isEqualToString:@""]:false;
    // only use the first image (for now.. maybe we can share in a loop?)
...
- (void) shareViaWeiBo:(CDVInvokedUrlCommand *)command{
    ...
    NSString *subject   = [command.arguments objectAtIndex:1]!= (id)[NSNull null]?[command.arguments objectAtIndex:1]:@"";
    NSArray  *filenames = [command.arguments objectAtIndex:2];
    NSString *share_url = [command.arguments objectAtIndex:3]!= (id)[NSNull null]?[command.arguments objectAtIndex:3]:@"";
    NSString* urlImage;
    for (NSString* filename in filenames) {
        urlImage = filename;
        break;
    ...
    if (!weiboAppid || !weiboRedirectURI) {
        CDVPluginResult * pluginResult = [CDVPluginResult resultWithStatus:CDVCommandStatus_ERROR messageAsString:@"未配置微博SDK参数"];
        [self.commandDelegate sendPluginResult:pluginResult callbackId:command.callbackId];
        return;
    }
    ...
    if([share_url isEqualToString:@""] && ![urlImage isEqualToString:@""]){
        //share image
        UIImage *img=[self getImage:urlImage];
        if(img){
            WBImageObject *imgObj=[WBImageObject object];
            imgObj.imageData=UIImageJPEGRepresentation(img, 0.8);
            msg.imageObject=imgObj;
        }else{
            [self _sendResult:CDVCommandStatus_ERROR :@"图片获取失败" :command.callbackId];
            return;
        }

    }else if(![share_url isEqualToString:@""]){
        WBWebpageObject *webObj=[WBWebpageObject object];
        webObj.webpageUrl=share_url;
        webObj.title=subject;
        webObj.description=message;
        webObj.objectID=subject;
        UIImage *img=[self getImage:urlImage];
        if(img){
            img=[self reSizeImage:img toSize:CGSizeMake(100, 100)];
            webObj.thumbnailData=UIImageJPEGRepresentation(img, 0.8);
        }
        msg.mediaObject=webObj;
    }else{
        [self _sendResult:CDVCommandStatus_ERROR :@"微博分享内容错误" :command.callbackId];
        return;
    }
    WBSendMessageToWeiboRequest *req=[WBSendMessageToWeiboRequest requestWithMessage:msg authInfo:nil access_token:_weibo_access_token];
    req.userInfo=@{@"callbackId":command.callbackId};
    if(![WeiboSDK sendRequest:req]){
        [self _sendResult:CDVCommandStatus_ERROR :@"微博分享传送失败" :command.callbackId];
        return;
    }

}

- (void) _sendResult:(CDVCommandStatus)status :(NSString *)msg :(NSString *)callbackId{
    CDVPluginResult * pluginResult = [CDVPluginResult resultWithStatus:status messageAsString:msg];
    ...
    if ([response isKindOfClass:WBAuthorizeResponse.class]) {
        ...
        if (res.requestUserInfo) {
            ...
            if(res.statusCode==WeiboSDKResponseStatusCodeSuccess){
                [self _sendResult:CDVCommandStatus_OK :@"成功分享到微博" :callbackId];
            }else{
                [self _sendResult:CDVCommandStatus_ERROR :@"分享失败" :callbackId];
            }
        }
    }
